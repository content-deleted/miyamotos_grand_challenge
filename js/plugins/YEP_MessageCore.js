...
Window_NameBox.prototype.initialize = function(parentWindow) {
    ...
    this.refresh();
    this.hide();
    this._sfont = VictorEngine.SFont.getSFont(35);
};
...
Window_NameBox.prototype.windowHeight = function() {
    return this.isTransparent() ? this.fittingHeight(1) : 50;
};
...
Window_NameBox.prototype.textColor = (n) => VictorEngine.SFont.getSFont(35);
Window_NameBox.prototype.changeTextColor = (n) => {};

// I wrote this for no reason i hate working iwth yanflys stuff so much
Window_NameBox.prototype.getBGWidth = function() {
    return this._text ? this._text.trim().split('').reduce(((col, x) => col + (x == ' ' ? 8 : 15)), 0) : 0;
};

Window_NameBox.prototype.contentsWidth = function() {
    return this.width;
};

Window_NameBox.prototype.isTransparent = function() {
   return this._parentWindow.opacity != 255 || $gameSystem.windowskin() == "Window_CYOA";
}
...
Window_NameBox.prototype.refresh = function(text, position) {
    ...
    // set namebox font
    let font = VictorEngine.SFont.getSFont(63);
    let y = 0;
    // back for the namebox
    if(this.isTransparent()) {
        this.setBackgroundType(2);
        font = VictorEngine.SFont.getSFont(35);
        this.contents._context.fillStyle = 'black';
        this.contents.fillRect(0, 8, this.width, 26);
        y = 2;
    } else {
    ...
    this.contents.sfont = font;
    this.drawTextEx(this._text.toUpperCase(), 3, y, this.width);
...
//     var target = SceneManager._scene._windowLayer;
//     target.addChild(this._nameWindow);
// };
...
Window_Message.prototype.clickTimer = 0;
Window_Message.prototype.isFastForward = function() {
    return this.doubleClicked || this.printAll || Input.isPressed("$START") || Input.isPressed("#backspace");
};

Yanfly.Message.Window_Message_update = Window_Message.prototype.update;
Window_Message.prototype.update = function() {
    //this.clickTimer--;
    // text skip with cancel / b
    ...
    Yanfly.Message.Window_Message_update.call(this);
    this.doubleClicked = false;
    // this.printAll = false;
    
    if(this.isOpen() && $gameSystem.isFastFowardEnabled() && !this.isAnySubWindowActive()) {
      if(Input.isPressed("$START") || Input.isPressed("#backspace")) {
        if(!this._previousFrameFastforwarding) {
          this._previousFrameFastforwarding = true;
          GameHam.StartVHSEffect();
        }
      } else {
        if(this._previousFrameFastforwarding) {
          GameHam.EndVHSEffect();
        }
        this._previousFrameFastforwarding = false;
      }
    } else {
      if(this._previousFrameFastforwarding) {
        GameHam.EndVHSEffect();
...
Yanfly.Message.Window_Message_clearflags = Window_Message.prototype.clearFlags;
Window_Message.prototype.clearFlags = function() {
    this.clickTimer = 0;
    this.doubleClicked = false;
    this.printAll = false;
    Yanfly.Message.Window_Message_clearflags.call(this);
}
