...
Game_System.prototype.getMessageSound = function() {
    ...
    const name = Array.isArray(this._msgSoundName) ? this._msgSoundName.pick() : this._msgSoundName;
    var obj = {
      name: name,
...
Window_Message.prototype.updateMessage = function() {
  ...
  if (state) {
    ...
    if (this._textState) {
      let t = this._textState.text[this._textState.index];
      if(t) {
        if (this._soundCount-- <= 0 && t.trim() != '' && !(['.','!','?',''].includes(t))) {
...
Window_Message.prototype.startWait = function(count) {
    ...
    if(count > KR.textSpeed) this._soundCount = 0;
};
